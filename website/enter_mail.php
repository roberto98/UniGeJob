...
<div class="container">
		...
		<form action="php/send_mail_recovery.php" method="POST" onsubmit="return check_input_Entermail();">
			<div class="center contents">

            		<ul class="ul_infield" style="width:100%;">
										<h1 style="text-align: left;">Recupero Password</h1>
										<br>

										<p class="wordwrap" style="text-align: left;">
											Inserisci qua sotto la tua mail. Ti invieremo un link da cliccare per reimpostare la password
										</p>
            				<li class="li_infield">
								...
								<button type="submit" name="reset_password"> Entra </button>
			</form>

						<p class="errorLog"><br><?php if(isset($_SESSION['NoMail'])){
																						echo htmlspecialchars($_SESSION['NoMail'], ENT_QUOTES);
																						unset($_SESSION['NoMail']);}

																					if(isset($_SESSION['TokenNotSent'])){
																						echo htmlspecialchars($_SESSION['TokenNotSent'], ENT_QUOTES);
																						unset($_SESSION['TokenNotSent']);}
																		?>
						</p>
